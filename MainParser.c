...
/* External objects */
extern india_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern india_int line; /* source code line number - defined in scanner.c */
...
extern india_nul startParser(void);
extern india_int startScanner(BufferPointer sc_buf);

static india_nul printParserError(char* fmt, ...);
static india_nul displayParser(BufferPointer ptrBuffer);
static india_intL getParserFileSize(char* fname);
static india_nul callGarbageCollector(void);
...
india_nul printParserError(india_chr* fmt, ...) {
...
india_intL getParserFileSize(india_chr* fname) {
	FILE* input;
	india_intL flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
india_nul displayParser(Buffer* ptrBuffer) {
...
india_nul callGarbageCollector(india_nul) {
